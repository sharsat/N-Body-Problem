...
createBodies_l = function (f) {
	bodies_l[0] = new body_e(f, 100, 60, 0, 0, -2, 0, 20); //-2.9
	bodies_l[1] = new body_e(f, -100, 60, 0, 0, 2, 0, 20);
	bodies_l[2] = new body_e(f, 0, 150, 150, 2, 0, 0, 20);


	// console.log(bodies_l[0].pos)
	bodies_l[0].pos.add(bodies_l[0].vel.copy().mult(dt / 2));
	bodies_l[1].pos.add(bodies_l[1].vel.copy().mult(dt / 2));
	bodies_l[2].pos.add(bodies_l[2].vel.copy().mult(dt / 2));
	bodies_l[0].calculateAcc(bodies_l[1]);
	bodies_l[0].calculateAcc(bodies_l[2]);
	bodies_l[1].calculateAcc(bodies_l[0]);
	bodies_l[1].calculateAcc(bodies_l[2]);
	bodies_l[2].calculateAcc(bodies_l[0]);
	bodies_l[2].calculateAcc(bodies_l[1]);
...
drawBodies_l = function (f) {
	trackBody0_l +=
		String(bodies_l[1].pos.x - centerOfMass_l(f).x) +
		' ' +
		String(bodies_l[1].pos.y - centerOfMass_l(f).y) +
		' ' +
		String(bodies_l[1].pos.z - centerOfMass_l(f).z) +
		'\n';
